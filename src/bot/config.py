...
class Config(BaseSettings):
    ...
    enable_prometheus: bool = Field(False, alias="ENABLE_PROMETHEUS")
    metrics_port: int = Field(9100, alias="METRICS_PORT")

    alert_bot_token: str | None = Field(None, alias="ALERT_BOT_TOKEN")
    alert_secret: str | None = Field(None, alias="ALERT_SECRET")
    alert_chat_ids: List[int] = Field(default_factory=list, alias="ALERT_CHAT_IDS")
    ...
    @classmethod
    def _parse_chats(cls, value: str | List[int]) -> List[int]:
        if isinstance(value, str):
